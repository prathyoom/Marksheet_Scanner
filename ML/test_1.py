...
def preprocess(img):
    ...
    img.save("marksheets/corrected.jpg")
    img = cv2.imread("marksheets/corrected.jpg")
    ...
    for line in lines:
        if len(line) > 0:
            ...
            for i in range(len(words)):
                ...
                result = difflib.get_close_matches(words[i], keywords, 1, 0.85)
                if len(result) > 0:
                    key = result[0]
    ...
    for k in keywords:
        for str in l[k]:
            if k == "board":
                ...
                result = difflib.get_close_matches("board", wrds, 1, 0.85)
                i = 0
...
def main():
    ...
    for row in reader:
        img_name = row[3]
        img = Image.open("marksheets/" + img_name)
